...
* 通过`.async`修饰符
  ...
  * 使用.async修饰符（**注意点：**emit触发事件中传的参数必须和.async前面绑定的数据一样）
...
![爷爷组件属性传递](.\imgs\爷爷组件属性传递.png)

![爷爷组件属性传递2](.\imgs\爷爷组件属性传递2.png)

* 在`Father`组件中通过`$attrs`获得组件`props`传递的属性, 通过`v-bind='$attrs'`将`Father`组件`props`获得的属性传递给`Child`孙子组件

  ![爸爸组件传递属性1](.\imgs\爸爸组件传递属性1.png)

* 在孙子组件中直接通过$attrs获得传递的属性

  ![孙子组件获取属性](.\imgs\孙子组件获取属性.png)

  ![孙子组件获取属性2](.\imgs\孙子组件获取属性2.png)
...
![爷爷组件传递方法](.\imgs\爷爷组件传递方法.png)

* 在`Father`父组件中使用`v-on='$listeners'`传递这个绑定事件方法到子组件

  ![父组件事件传递1](.\imgs\父组件事件传递1.png)

* 在`child`子组件中可以直接通过`$listeners.事件名称(参数)`或者`$emit('事件名称',参数)`调用绑定事件方法

  ![子组件获取事件绑定方法](.\imgs\子组件获取事件绑定方法.png)
...
* 在爷爷组件`Grandpa.vue`中通过provide注入数据
  ...
  ![爷爷provide](.\imgs\爷爷provide.png)

* 在孙子组件`Child.vue`中通过`inject`接收到这个爷爷组件注入的数据

  ![孙子组件inject接收](.\imgs\孙子组件inject接收.png)
...
* 在`Father.vue`中的属性和方法
  ...
  ![$refs3](.\imgs\$refs3.png)

* 在`Father.vue`上添加$ref属性

  ![$refs1](.\imgs\$refs1.png)
...
![兄弟组件传值1](.\imgs\兄弟组件传值1.png)

> `Child2`组件通过$on进行事件监听

![兄弟组件传值](.\imgs\兄弟组件传值.png)

> `Child`组件通过$emit进行事件触发

![兄弟组件传值2](.\imgs\兄弟组件传值2.png)
